...
try {
    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          help: (options && options.play && options.play.help) || "Anhada à lista de reproduçom umha cançom",
        ...
        this.help = {
          ...
          help: (options && options.help && options.help.help) || "Ajuda para os comandos",
        ...
        this.pause = {
          ...
          help: (options && options.pause && options.pause.help) || "Pausa a música",
        ...
        this.resume = {
          ...
          help: (options && options.resume && options.resume.help) || "Reproduz a cançom pausada",
        ...
        this.leave = {
          ...
          help: (options && options.leave && options.leave.help) || "Abandona a canle",
        ...
        this.queue = {
          ...
          help: (options && options.queue && options.queue.help) || "Ver lista de reproduçom atual",
        ...
        this.np = {
          ...
          help: (options && options.np && options.np.help) || "Ver a cançom que se está a reproduzir",
        ...
        this.loop = {
          ...
          help: (options && options.loop && options.loop.help) || "Infinita música",
        ...
        this.search = {
          ...
          help: (options && options.search && options.search.help) || "Faz umha perquisa no youtube",
        ...
        this.clearqueue = {
          ...
          help: (options && options.clear && options.clear.help) || "Limpa a lista de reproduçom atual",
        ...
        this.volume = {
          ...
          help: (options && options.volume && options.volume.help) || "Cambia o volume",
        ...
        this.remove = {
          ...
          help: (options && options.remove && options.remove.help) || "Remova umha cançom da lista de reproduçom",
        ...
        this.skip = {
          ...
          help: (options && options.skip && options.skip.help) || "Remova umha cançom com `skip [number]`",
        ...
        this.shuffle = {
            ...
            help: (options && options.shuffle && options.shuffle.help) || "Mistura a lista de reproduçom",
        ...
        this.deleteQueue = {
            ...
            help: (options && options.deleteQueue && options.deleteQueue.help) || "Remova a lista de reproduçom",
      ...
      checkVoice(mem, bot) {
        return new Promise((resolve, reject) => {
          ...
          if (!mem.voiceChannel) reject("Nom estás numha canle de voz!");
          if (bot.voiceChannel) {
            if (bot.voiceChannel.id == mem.voiceChannel.id) resolve(mem.voiceChannel)
            else reject("Estás numha canle de voz diferente!")
          } else {
          ...
          if (tCmd.enabled) {
            if (!musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(tCmd.masked)) {
              if (musicbot.recentTalk.has(msg.author.id)) {
                if (musicbot.cooldown.enabled == true && !musicbot.cooldown.exclude.includes(tCmd.masked)) return msg.channel.send(musicbot.note("fail", "Deves esperar para usar os comandos da música novamente!"));
              }
    ...
    musicbot.playFunction = (msg, suffix, args, ignore) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Nom estas numha canle de voz.`));
      ...
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Deves estar na mesma canle de voz que eu.`));
      if (q.songs.length >= musicbot.maxQueueSize && musicbot.maxQueueSize !== 0) return msg.channel.send(musicbot.note('fail', 'Tamanho máximo da lista de reproduçom!'));
      ...
      if (searchstring.startsWith('http') && searchstring.includes("list=")) {
        msg.channel.send(musicbot.note("search", `Pesquisando itens da lista de reproduçom~`));
        ...
        ytpl(playid, {limit: musicbot.maxQueueSize}, function(err, playlist) {
          if(err) return msg.channel.send(musicbot.note('fail', `Ocorreu um erro ao buscar essa lista de reproduçom!`));
          if (playlist.items.length <= 0) return msg.channel.send(musicbot.note('fail', `Nom foi possível obter nenhum vídeo dessa lista de reproduçom.`));
          ...
          playlist.items.forEach(async (video) => {
            ...
            if (ran >= playlist.items.length) {
              ...
              if (index == 0) msg.channel.send(musicbot.note('fail', `Nom foi possível obter nenhum vídeo dessa lista de reproduçom!`))
              else if (index == 1) msg.channel.send(musicbot.note('note', `Adicionada umha cançom a lista de reproduçom.`));
              else if (index > 1) msg.channel.send(musicbot.note('note', `Na lista de reproduçom ${index} cançons.`));
            }
          });
        });
      } else {
        if (!ignore) msg.channel.send(musicbot.note("search", `\`Perquisando: ${searchstring}\`~`));
        new Promise(async (resolve, reject) => {
          ...
          if (!res) return msg.channel.send(musicbot.note("fail", "Algo foi mal, tente novamente!"));
          ...
          if (!ignore) {
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              ...
              try {
                embed.setAuthor('Adicionando a lista de reproduçom', client.user.avatarURL);
                ...
                embed.addField("Adicionado a lista de reproduçom", res.queuedOn, musicbot.inlineEmbeds);
                ...
                if (musicbot.requesterName && resMem) embed.setFooter(`Adicionado por ${client.users.get(res.requester).username}`, res.requesterAvatarURL);
                if (musicbot.requesterName && !resMem) embed.setFooter(`Adicionado por \`UnknownUser (ID: ${res.requester})\``, res.requesterAvatarURL);
                msg.channel.send({
                  embed
                });
              } catch (e) {
              ...
              try {
                ...
                .replace(/~/g, '\\~')
                .replace(/`/g, '\\`');
                msg.channel.send(`Agora reproduzindo: **${songTitle}**\nAdicionada por: ${client.users.get(res.requester).username}\nQueued On: ${res.queuedOn}`)
              } catch (e) {
    ...
    musicbot.helpFunction = (msg, suffix, args) => {
      ...
      if (!suffix) {
        ...
        if (msg.channel.permissionsFor(msg.guild.me)
          ...
          if (command.exclude) return msg.channel.send(musicbot.note('fail', `${suffix} nom é um comando válido!`));
        ...
        msg.channel.send(musicbot.note('fail', `${suffix} nom é um comando válido!`));
      };
    };

    musicbot.skipFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nom estás numha canle de voz.`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Nenhuma música está sendo reproduzida.'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Deves estar na mesma canle de voz na que estou.`));
      ...
      if (!dispatcher || dispatcher === null) {
        ...
        return msg.channel.send(musicbot.note("fail", "Algo foi mal ao fazer skip."));
      };
      if (voiceConnection.paused) dispatcher.end();
      dispatcher.end();
      msg.channel.send(musicbot.note("note", "Cançom skipeada."));
    };

    musicbot.pauseFunction = (msg, suffix, args) => {
      if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `Nom estás numha canle de voz.`));
      const voiceConnection = client.voiceConnections.find(val => val.channel.guild.id == msg.guild.id);
      if (voiceConnection === null) return msg.channel.send(musicbot.note('fail', 'Nom há música sendo reproduzida.'));
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Deves estar na mesma canle de voz na que estou..`));
      ...
      msg.channel.send(musicbot.note('note', 'Música já pausada.'));
    };
    ...
    musicbot.resumeFunction = (msg, suffix, args) => {
      ...
      if (voiceConnection && voiceConnection.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Deves estar na mesma canle de voz que estou eu.`));
      ...
      const dispatcher = voiceConnection.player.dispatcher;
      if (!dispatcher.paused) return msg.channel.send(musicbot.note('fail', `A música já se está a reproduzer.`))
    ...
    musicbot.leaveFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanLeave === true) {
        ...
        musicbot.emptyQueue(msg.guild.id).then(() => {
          ...
          msg.channel.send(musicbot.note('note', 'Saiu da canle com sucesso.'));
        }).catch((res) => {
    ...
    musicbot.npFunction = (msg, suffix, args) => {
      ...
      if (musicbot.queues.get(msg.guild.id).songs.length <= 0) return msg.channel.send(musicbot.note('note', 'Lista de reproduçom baleira.'));

      if (msg.channel.permissionsFor(msg.guild.me)
        .has('EMBED_LINKS')) {
        const embed = new Discord.RichEmbed();
        try {
          embed.setAuthor('Agora se está a reproduzir', client.user.avatarURL);
          ...
          if (musicbot.requesterName && resMem) embed.setFooter(`Adicionada por ${client.users.get(queue.last.requester).username}`, queue.last.requesterAvatarURL);
          if (musicbot.requesterName && !resMem) embed.setFooter(`Adicionada por \`UnknownUser (ID: ${queue.last.requester})\``, queue.last.requesterAvatarURL);
          msg.channel.send({
            embed
          });
        } catch (e) {
        ...
        try {
          var songTitle = queue.last.title.replace(/\\/g, '\\\\')
            ...
            .replace(/~/g, '\\~')
            .replace(/`/g, '\\`');
          msg.channel.send(`Agora se está a reproduzir: **${songTitle}**\nAdicionada por: ${client.users.get(queue.last.requester).username}\nQueued On: ${queue.last.queuedOn}`)
        } catch (e) {
          console.error(`[${msg.guild.name}] [npCmd] ` + e.stack);
        };
      }
    ...
    musicbot.deleteQueueFunction = async (msg, suffix, args) => {
      ...
      musicbot.emptyQueue(msg.guild.id).then(() => {
        ...
        return msg.channel.send(musicbot.note("note", "A lista de reproduçom debe estar baleira."))
      }).catch(async (res) => {
    ...
    musicbot.queueFunction = (msg, suffix, args) => {
      ...
      if (suffix) {
        ...
        if (musicbot.requesterName && resMem) embed.setFooter(`Adicionada por ${client.users.get(video.requester).username}`, video.requesterAvatarURL);
        if (musicbot.requesterName && !resMem) embed.setFooter(`Adicionada por \`UnknownUser (ID: ${video.requester})\``, video.requesterAvatarURL);
        ...
        if (queue.songs.length > 11) {
          ...
          msg.channel.send(embed).then(m => {
            m.react('âª').then( r => {
              ...
              let backFilter = m.createReactionCollector((reaction, user) => reaction.emoji.name === 'âª' && user.id === msg.author.id, { time: 120000 });
    ...
    musicbot.searchFunction = (msg, suffix, args) => {
      if (msg.member.voiceChannel === undefined) return msg.channel.send(musicbot.note('fail', `Nom estás numha canle de voz.`));
      let vc = client.voiceConnections.find(val => val.channel.guild.id == msg.member.guild.id)
      if (vc && vc.channel.id != msg.member.voiceChannel.id) return msg.channel.send(musicbot.note('fail', `Deves estar na mesma canle de voz que estou eu.`));
      ...
      msg.channel.send(musicbot.note('search', `Perquisando: \`${searchstring}\``))
        .then(response => {
          musicbot.searcher.search(searchstring, {
            ...
            .then(searchResult => {
              ...
              const startTheFun = async (videos, max) => {
                if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
                  const embed = new Discord.RichEmbed();
                  embed.setTitle(`Escolha o vídeo`);
                  embed.setColor(musicbot.embedColor);
                  var index = 0;
                  ...
                  embed.setFooter(`Adicionado por: ${msg.author.username}`, msg.author.displayAvatarURL);
                  msg.channel.send({
                    embed
                  })
                  .then(firstMsg => {
                    ...
                    .then(collected => {
                      ...
                      if (song_number >= 0) {
                        ...
                        if (musicbot.requesterName && resMem) embed.setFooter(`Adicionada por ${client.users.get(videos[song_number].requester).username}`, videos[song_number].requesterAvatarURL);
                        if (musicbot.requesterName && !resMem) embed.setFooter(`Adicionada por \`UnknownUser (ID: ${videos[song_number].requester})\``, videos[song_number].requesterAvatarURL);
                        msg.channel.send({
                          embed
                        }).then(() => {
    ...
    musicbot.removeFunction = (msg, suffix, args) => {
      ...
      if (test) {
        ...
        musicbot.updatePositions(musicbot.queues.get(msg.guild.id), msg.guild.id).then(res => {
          ...
          msg.channel.send(musicbot.note("note", `Eliminado:  \`${test.title.replace(/`/g, "'")}\``));
        }).catch(e=> {
          console.log(e)
    ...
    musicbot.executeQueue = (msg, queue) => {
      ...
      new Promise((resolve, reject) => {
          ...
          if (musicbot.messageNewSong == true && queue.last && queue.loop !== "song") {
            ...
            if (msg.channel.permissionsFor(msg.guild.me).has('EMBED_LINKS')) {
              const embed = new Discord.RichEmbed()
              .setTitle("Agora se está a reproduzir", `${req !== null ? req.displayAvatarURL : null}`)
              ...
              .setColor(musicbot.embedColor)
              .setFooter(`Adicionada por ${req !== null ? req.username : "Unknown User"}`, `${req !== null ? req.displayAvatarURL : null}`);
              msg.channel.send({embed});
            } else {
